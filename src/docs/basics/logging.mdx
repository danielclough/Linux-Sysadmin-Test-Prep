...
1. Change cron logging for all levels in cron.log
...
`logger [options] [message]`
    `-p, --priority`
	- `logger -p cron.info " This is a info message from cron"`

### Quiz
1. Create a rule for `local4` critical level logs to be sent to `/var/log/local4-info.log`
2. Use logger to send a message from local4 of critical level to the log
