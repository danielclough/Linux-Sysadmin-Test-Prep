...
1. `dpkg --get-selections > ~/packagelist.txt` (Export the list of installed packages to a file)
1. `cp -Rp /etc/apt/sources.list* ~/sources` (Backup your sources)
1. `apt-key exportall > ~/trusted_keys.txt` (Backup  trusted key list)
1. Backup configuration files you want to save.

### Restore

1. `sudo apt-key add ~/trusted_keys.txt`
1. `sudo cp -R ~sources/* /etc/apt/`
