...
`lxc remote list` (Show remote servers that can be used as `<imageserver>`)

`lxc image list images` (Show local images)

`lxc image list images:` (Show all available images)

`lxc image list images: debian amd64` (Show available amd64 debian images)
...
`lxc list` (Show local instances with network info)
...
`lxc file push <instancename>/path/in/container path/on/host`
